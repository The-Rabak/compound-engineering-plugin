...
- Task data-integrity-guardian(branch diff content) - Reviews migration safety, constraint naming, and migration conventions
- Task data-migration-expert(branch diff content) - Validates ID mappings match production, checks for swapped values, verifies rollback safety
- Task deployment-verification-agent(branch diff content) - Creates Go/No-Go deployment checklist with SQL verification queries
