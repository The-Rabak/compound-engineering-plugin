...
2. Run lightweight repo research in parallel:
   - Before dispatching `repo-research-analyst` or `learnings-researcher`, apply the shared `Named Agent Dispatch` protocol from `commands/workflows/references/orchestration-protocol.md`. Verify each agent metadata/source, resolve the concrete subagent identifier, and pass only the research payload. Do not read or paste full agent bodies into prompts.
