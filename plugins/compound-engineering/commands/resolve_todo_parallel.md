...
Before dispatching `pr-comment-resolver`, apply the shared `Named Agent Dispatch` protocol from `commands/workflows/references/orchestration-protocol.md`. Verify the agent metadata/source, resolve the concrete subagent identifier, and pass only the todo-specific payload. Do not read or paste the full agent body into the prompt.
