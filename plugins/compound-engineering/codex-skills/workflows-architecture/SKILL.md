...
This phase sits **after** `$workflows-plan skill` and **before** `$deepen-plan skill`.
...
- Record the chosen review depth in the artifact so `$deepen-plan skill`, `$workflows-work skill`, and `$workflows-review skill` inherit the same context.
...
- For escalated runs, apply the shared `Named Agent Dispatch` protocol from `references/orchestration-protocol.md`.
...
9. **Translate findings into downstream guidance** -- what `$deepen-plan skill`, `$workflows-work skill`, and `$workflows-review skill` should preserve or verify.
...
- **Clear next step**: run `$deepen-plan skill` with the updated plan
...
Next: Run `$deepen-plan skill <plan_path>` so execution hardening uses this architecture artifact.
```
