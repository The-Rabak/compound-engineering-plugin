...
## Arguments
[bug report, failing command, issue URL, or symptom description]
...
- `$reproduce-bug skill` for reproduction and evidence gathering
...
Use `$reproduce-bug skill` unless you already have a stable local reproduction and fresh evidence from the current run.
...
If the fix expands into broader feature or cross-boundary work, stop the debugging run and hand off to `$workflows-plan skill`, `$workflows-architecture skill`, `$deepen-plan skill`, or `$workflows-to-issues skill` with a concise debug summary instead of letting the debug session sprawl.
...
- `$workflows-brainstorm skill` for reframing the problem
- `$workflows-architecture skill` for boundary and contract corrections
- `$deepen-plan skill` or `$workflows-to-issues skill` when the fix needs a planned execution path
