...
## Arguments
[optional: project standards, constraints, or amendment goal]
...
- **`$workflows-brainstorm skill`** -- read the constitution before idea shortlisting or approach exploration, score candidate directions for constitution fit, and record any proposed amendment instead of silently drifting
- **`$workflows-plan skill`** -- record `constitution_version` and any `constitution_waivers`, then translate relevant rules into acceptance criteria and approvals
- **`$workflows-work skill`** -- inject constitution guardrails into execution prompts and stop for approval when the constitution requires it
- **`$workflows-review skill`** -- treat unwaived constitution violations as blocking

Then offer next steps:

1. Proceed to `$workflows-brainstorm skill`
2. Proceed to `$workflows-plan skill`
3. Done for now
