...
1. **Code Review (`$workflows-review skill`)**: If NOT already on the target branch (PR branch or requested branch), offer worktree for isolated review
2. **Feature Work (`$workflows-work skill`)**: Always ask if user wants parallel worktree or live branch work
...
The skill is automatically called from `$workflows-review skill` and `$workflows-work skill` commands:
