...
- **`bulk`** (default) -- Review happens after ALL tasks complete, using `$workflows-review skill`. This is the standard behavior and the only mode where named review agents run.
...
- **`both`** -- Inline review per task AND comprehensive `$workflows-review skill` at the end. Maximum quality assurance.
...
**Hard rule:** Named review agents belong to `$workflows-review skill`. `$workflows-work skill` may coordinate inline template-based checks, but it must never bypass `$workflows-review skill` by dispatching named reviewers directly.
...
1. **Read the work document and extract WHY + guardrail context**
    ...
    - If the input is a ticket index or ticket file, first load `references/ticket-execution-contract.md` and verify the artifact includes the required index or ticket contract. Stop and send the user back to `$workflows-to-issues skill` if the contract is missing or malformed.
    ...
    - Check for `handoff:` frontmatter in the parent plan. If present, verify all flags are `true` (problem_narrative, user_story, architectural_context, success_criteria). If any are `false`, warn the user that WHY context is incomplete and suggest running `$workflows-brainstorm skill` or `$workflows-plan skill` first.
...
If the plan lacks structural details, the ticket index or ticket lacks the ticket execution contract, or no architecture artifact / handoff contract can explain the boundaries, refuse to proceed and suggest running `$workflows-architecture skill` first if the boundaries are still fuzzy, then `$deepen-plan skill` or `$workflows-to-issues skill`, or manually repairing the execution packet.

If the plan lacks the `tdd` block or `## TDD & Evidence Contract`, or if the resolved contract is ambiguous, refuse to proceed and suggest `$workflows-plan skill` or `$deepen-plan skill` to repair the execution contract before spawning subagents.
...
- Review guidance: [what `$workflows-review skill` must verify later]
...
If the `--review-mode` argument is `inline` or `both`, perform a two-stage inline review before proceeding to the next unit. If `--review-mode` is `bulk` (the default), skip this step.
   ...
   **Stage 2: Code Quality Review** (only after spec compliance passes)
   ...
   - If **FAIL** with only Important/Minor issues: log them for the orchestrator's attention but proceed to next task (these will also be caught by `$workflows-review skill` if run later)

   **Note:** Inline review is a lightweight per-unit check. It does NOT replace the comprehensive `$workflows-review skill` multi-agent review. When `--review-mode both` is active, inline review runs per-unit AND `$workflows-review skill` runs after all units complete.
...
3. **Run `$workflows-review skill` when named reviewers are needed**

   Use `$workflows-review skill` for complex, risky, or large changes, and whenever `review_mode` requires final reviewer-agent coverage (`bulk` or `both`). Do not dispatch named review agents directly from `$workflows-work skill`.

   `$workflows-work skill` may only run the inline template-based reviewers described above. All named agents from `compound-engineering.local.md` frontmatter (`review_agents`) must be coordinated by `$workflows-review skill`, which owns the template-loading protocol, WHY-context injection, mandatory always-on reviewers, and conditional reviewer rules.
...
- When reproduction is fuzzy, prior attempts may matter, or the issue may need a fix-vs-diagnosis-vs-design decision, route through `$workflows-debug skill` instead of improvising a longer failure loop
