...
When that happens, recommend `$workflows-debug skill` for orchestration or escalate to `$workflows-brainstorm skill`, `$workflows-architecture skill`, or `$deepen-plan skill` as appropriate.
...
This information feeds into the learnings brief for subsequent tasks and can be captured in `docs/solutions/` via `$workflows-compound skill`.
