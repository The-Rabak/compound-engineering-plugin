...
Follow the same interaction style as `$workflows-brainstorm skill`:
...
Run `$workflows-compound skill` after solving problems to start building your knowledge base.
```
...
3. have it write the new learning following the `$workflows-compound skill` or `compound-docs` document structure: frontmatter, problem, root cause, current solution, code examples, and prevention tips
...
3. recommend running `$workflows-compound skill` after the next real encounter with that area
...
## Relationship to `$workflows-compound skill`

- `$workflows-compound skill` captures a newly solved, verified problem
- `$workflows-compound-refresh skill` maintains older learnings as the codebase evolves

Use **Replace** only when the refresh process has enough real evidence to write a trustworthy successor. When evidence is insufficient, mark as stale and recommend `$workflows-compound skill` for when the user next encounters that problem area.
