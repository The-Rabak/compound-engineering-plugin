...
Brainstorming answers **WHY** we're building something and **WHAT** it is. It produces the lynchpin artifacts -- a problem narrative, user story, and architectural context -- that thread through every downstream phase (`$workflows-plan skill` -> `$workflows-architecture skill` -> `$deepen-plan skill` -> `$workflows-work skill` -> `$workflows-review skill`).
...
Use **AskUserQuestion tool** to suggest: "Your requirements seem detailed enough to proceed directly to planning. Should I run `$workflows-plan skill` instead, or would you like to explore the idea further?"
...
Use `references/vertical-slice-architecture.md` as the source of truth for feature-home thinking and shared/global boundary language.
...
2. **Proceed to planning** - Run `$workflows-plan skill` (will auto-detect this brainstorm, then route through `$workflows-architecture skill` before `$deepen-plan skill`)
...
1. **Move to planning** - Continue to `$workflows-plan skill` with this document, then use `$workflows-architecture skill` as the supported next handoff before deepening
2. **Done for now** - Brainstorming complete. To start planning later: `$workflows-plan skill [document-path]`
...
Next: Run `$workflows-plan skill`, then `$workflows-architecture skill` before
`$deepen-plan skill` or `$workflows-work skill`.
...
- **`$workflows-plan skill`** -- Reads user story and architectural context to structure phases, while also checking constitution rules and recording any waivers or amendment proposals.
- **`$deepen-plan skill`** -- Uses problem narrative and success criteria to evaluate whether deepened tasks still serve the original intent.
- **`$workflows-work skill`** -- Feeds architectural context into each execution agent's `{{ARCHITECTURAL_CONTEXT}}` block via the canonical execution-agent template, while constitution rules act as guardrails for implementation and approvals.
- **`$workflows-review skill`** -- Uses problem narrative, user story, success criteria, and constitution baselines as the frame for evaluating whether the implementation actually solves the stated problem without policy drift. Named review agents are coordinated there, not dispatched ad hoc from other workflows.
