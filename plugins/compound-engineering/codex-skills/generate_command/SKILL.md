...
Create a new slash command in `.agents/skills/` for the requested task.
...
4. Verify
   - Run tests: use the project's test command (e.g., `npm test`, `pytest`, `cargo test`, `php artisan test`)
   - Run linter: use the project's linter (e.g., `eslint`, `ruff`, `clippy`, `pint`)
...
1. **Create the file** at `.agents/skills/[name].md` (subdirectories like `workflows/` supported)
