...
Ralph is the canonical red-green-refactor engine behind `$workflows-work skill` when the resolved TDD contract selects Ralph-driven execution. Use this command directly only when you need manual control outside the orchestrated work path; it is not a detached alternative workflow.
