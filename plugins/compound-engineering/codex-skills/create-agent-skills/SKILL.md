...
Custom slash commands have been merged into skills. A file at `.agents/skills/review.md` and a skill at `.agents/skills/review/SKILL.md` both create `/review` and work the same way. Existing `.agents/skills/` files keep working. Skills add optional features: a directory for supporting files, frontmatter to control invocation, and automatic context loading.
...
- **Named review agents** must be coordinated through `$workflows-review skill`, not spawned directly from ad hoc prompts.
