...
Interactive setup for `compound-engineering.local.md` -- configures which agents run during `$workflows-review skill` and `$workflows-work skill`, plus the visible Ralph/TDD defaults that planning and execution inherit.
...
options:
  - label: "Bulk (Default)"
    description: "Runs $workflows-review skill once at the end of all tasks. This is where named review agents run."
  - label: "Inline"
    description: "Runs template-based inline checks after each task. Does not authorize direct named review-agent dispatch."
  - label: "Both"
    description: "Inline checks per task plus $workflows-review skill at the end."
```
...
- `review_mode`: "bulk" (default), "inline", or "both" (`bulk`/`both` invoke `$workflows-review skill`; `inline` stays template-based and must not spawn named review agents directly)

**Workflow-injected mandatory reviewers:**
- `$workflows-review skill` always adds `agent-native-reviewer`, `learnings-researcher`, and `uncle-bob`, regardless of `review_agents`.
- `$workflows-architecture skill` always runs `architecture-strategist` and `uncle-bob`.
...
These notes are passed to `$workflows-review skill` and to the template-based review steps inside `$workflows-work skill`. They do not authorize direct named review-agent dispatch outside `$workflows-review skill`.
