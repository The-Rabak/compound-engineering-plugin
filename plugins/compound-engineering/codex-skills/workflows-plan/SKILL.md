...
7. **Enable architecture-first execution** -- `$workflows-architecture skill` turns the plan into a dedicated architecture artifact before `$deepen-plan skill`, `$workflows-work skill`, and `$workflows-review skill` harden or execute it
...
Plans consume the project constitution from `$workflows-constitution skill` when available, plus lynchpin artifacts from `$workflows-brainstorm skill` when available, or construct feature context fresh when running standalone. Either way, the plan document carries forward the WHY, WHERE, DONE, GUARDRAIL, TDD, and **execution shape** contract that all downstream phases depend on. After the plan is written, the next explicit step is `$workflows-architecture skill`, not direct deepening.
...
4. If the feature appears to conflict with the constitution, ask the user whether this should be:
   ...
   - a constitution amendment to be handled by `$workflows-constitution skill`
...
- Spawn the custom agent `repo-research-analyst` with task: feature_description
- Spawn the custom agent `learnings-researcher` with task: feature_description
...
- Spawn the custom agent `best-practices-researcher` with task: feature_description
- Spawn the custom agent `framework-docs-researcher` with task: feature_description
...
For plans that will be executed via `$workflows-work skill`, the plan must include the packet section required by the selected mode:
- `## Execution Slices`
...
- Spawn the custom agent `spec-flow-analyzer` with task: feature_description, user_story, success_criteria, research_findings
...
**Important for `$workflows-work skill` compatibility:** Every plan shape must still declare `execution_shape` and include the matching packet section from `references/execution-shape.md`.
...
**Execution Readiness (for `$workflows-work skill`):**
...
2. **Run `$workflows-architecture skill`** - Create the dedicated architecture improvement artifact in `docs/architecture/` and record the handoff contract
3. **Run `$deepen-plan skill`** - Enhance each section with architecture guidance plus parallel research agents after the architecture handoff is explicit
4. **Run `$workflows-to-issues skill`** - Create local ticket artifacts once the architecture handoff is explicit
5. **Review and refine** - Improve the document through structured self-review
6. **Start `$workflows-work skill`** - Begin implementing this plan locally once the architecture handoff is explicit
7. **Start `$workflows-work skill` on remote** - Begin implementing in Claude Code on the web once the architecture handoff is explicit (use `&` to run in background)
8. **Create Issue** - Create issue in project tracker

Based on selection:
- **Open plan in editor** → Run `open docs/plans/<plan_filename>.md` to open the file in the user's default editor
- **`$workflows-architecture skill`** → Call the $workflows-architecture skill command with the plan file path
- **`$deepen-plan skill`** → Call the $deepen-plan skill command with the plan file path only after architecture improvement is complete and `architecture_ref` or a labeled handoff artifact has been recorded
- **`$workflows-to-issues skill`** → Call `$workflows-to-issues skill` with the plan file path after architecture guidance is explicit; recommend this after `$deepen-plan skill`, but allow it directly after `$workflows-plan skill` when the user wants earlier backlog shaping
...
- **`$workflows-work skill`** → Call the $workflows-work skill command with the plan file path once the architecture artifact or explicit architecture handoff contract is available
- **`$workflows-work skill` on remote** → Run `$workflows-work skill docs/plans/<plan_filename>.md &` after the architecture handoff is explicit so execution agents do not guess at boundaries
- **Create Issue** → See "Issue Creation" section below
- **Other** (automatically provided) → Accept free text for rework or specific changes

**Note:** If running `$workflows-plan skill` with ultrathink enabled, automatically run `$workflows-architecture skill` and then `$deepen-plan skill` after plan creation for maximum depth and grounding.

Loop back to options after Simplify or Other changes until user selects `$workflows-work skill`.
...
2. **After creation:**
   - Ask if they want to proceed to `$workflows-architecture skill`, then `$deepen-plan skill`, or `$workflows-work skill` once the architecture handoff is explicit
...
**`$workflows-architecture skill`** reads:
...
**`$workflows-to-issues skill`** reads:
...
- **Named reviewer ownership** -- `$workflows-review skill` owns named review-agent coordination, template loading, and WHY-context injection for reviewer prompts
