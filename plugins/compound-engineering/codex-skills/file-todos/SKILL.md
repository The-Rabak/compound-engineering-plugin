...
**Use slash command:** `$workflows-triage skill` for research-backed interactive approval workflow
...
| Trigger | Flow | Tool |
|---------|------|------|
| Code review | `$workflows-review skill` → Findings → `$workflows-triage skill` → Todos | Review agent + skill |
| PR comments | `/resolve_pr_parallel` → Individual fixes → Todos | gh CLI + skill |
| Code TODOs | `$resolve_todo_parallel skill` → Fixes + Complex todos | Agent + skill |
