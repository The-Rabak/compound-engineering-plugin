...
This phase sits after `$workflows-plan skill` or `$deepen-plan skill` and before ticket-scoped execution. Its job is to convert a strategic plan into a **local kanban-style ticket set** with enough context for one focused execution run at a time.

The command is named `$workflows-to-issues skill` for continuity with the broader plan-to-issue pattern, but **v1 writes local artifacts only**. It does not publish tracker issues automatically.
...
If the selected `execution_shape` is missing or the packets are too vague to map cleanly into tickets, stop and send the user back to `$deepen-plan skill` instead of inventing ticket boundaries.
...
- surface uncertainty instead of hiding it when ticketizing directly after `$workflows-plan skill`
...
- Run `$workflows-work skill` on the generated `index.md` so execution can pick the next safe batch automatically, or target one ticket file manually when you need to force a narrower run.
```
