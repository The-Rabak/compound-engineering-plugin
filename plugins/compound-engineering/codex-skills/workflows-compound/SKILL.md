...
```bash
$workflows-compound skill                    # Document the most recent fix
$workflows-compound skill [brief context]    # Provide additional context hint
```
...
- **Execution session learnings**: Patterns and fixes discovered during `$workflows-work skill` task execution (from `docs/execution-sessions/`)
...
<manual_override> Use $workflows-compound skill [context] to document immediately without waiting for auto-detection. </manual_override> </auto_invoke>
...
- **Manual trigger**: User can invoke agents after $workflows-compound skill completes for deeper review
...
- `$workflows-plan skill` - Planning workflow (references documented solutions)
