...
- [ ] **`handoff` frontmatter** -- check all fields are `true`; if any are `false` or missing, flag: "Plan is missing [X]. Deepening may add technically correct but purpose-misaligned enhancements. Consider running `$workflows-plan skill` to fill gaps first."
...
- [ ] If an architecture path exists, read it and extract:
  ...
  - Drift Checks
  - Recommendations for `$deepen-plan skill`, `$workflows-work skill`, and `$workflows-review skill`
...
- [ ] Record whether deepening used a real artifact or a plan-derived handoff contract so `$workflows-work skill` and `$workflows-review skill` inherit the same structural guidance
- [ ] If no architecture artifact exists, continue but flag: "No architecture artifact found. Consider running `$workflows-architecture skill` before deepening so structural decisions are explicit."
...
<thinking>
Check if the plan has sufficiently structured execution packets for the subagent orchestration model in `$workflows-work skill`. Use `references/execution-shape.md` as the source of truth. Plans need packets that are independently executable, testable, and traceable back to the user story without forcing fake verticality.
</thinking>
...
- [ ] If the mode is `vertical-slices`, also apply `references/vertical-slice-architecture.md` for feature-home and shared/global boundary checks
...
| 0-49% | Plan needs significant restructuring. Add an "Execution Readiness" enhancement pass that decomposes vague phases/tasks into the packet shape required by the selected mode. **Note:** `$workflows-work skill` will refuse to execute plans that lack a coherent execution shape unless the user explicitly approves a mode change or legacy adaptation. |
...
**This validation ensures the plan is ready for `$workflows-work skill`'s subagent orchestration model**, where each packet is delegated to a focused subagent with clear scope, proof, and termination criteria.
...
# 2. User's platform-global skills (for example ~/.codex/skills or ~/.config/opencode/skills)
...
<thinking>
Check for documented learnings from $workflows-compound skill. These are solved problems stored as markdown files. Spawn a sub-agent for each learning to check if it's relevant.
</thinking>

**LEARNINGS LOCATION - Check these exact folders:**

```
docs/solutions/           <-- PRIMARY: Project-level learnings (created by $workflows-compound skill)
├── performance-issues/
│   └── *.md
├── debugging-patterns/
│   └── *.md
...
2. **Learnings/Solutions sub-agents** - Relevant documented learnings from $workflows-compound skill
...
3. **Start `$workflows-work skill`** - Begin implementing this enhanced plan with its architecture artifact or explicit handoff contract
...
- **`$workflows-work skill`** → Call the $workflows-work skill command with the plan file path so execution loads the same architecture artifact or explicit handoff contract
- **Deepen further** → Ask which sections need more research, then re-run those agents
- **Revert** → Restore from git or backup

## Example Enhancement

**Before (from $workflows-plan skill):**
```markdown
## Technical Approach
