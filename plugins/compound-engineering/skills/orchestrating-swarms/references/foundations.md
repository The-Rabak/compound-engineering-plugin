...
| Team | Named group of collaborating agents with a leader and teammates. | `~/.claude/teams/{name}/config.json` |
...
| Task | Shared work item with owner, status, and dependencies. | `~/.claude/tasks/{team}/N.json` |
| Inbox | Message store used for teammate communication. | `~/.claude/teams/{name}/inboxes/{agent}.json` |
