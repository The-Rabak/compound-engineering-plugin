...
describe("ticket flow review contract", () => {
  test("registers the reusable ticket-flow reviewer and wires it into review", async () => {
    ...
    expect(reviewPrompt).toContain("[--batches N-M]")
    expect(reviewPrompt).toContain("this is a ticket-window review")
    expect(reviewPrompt).toContain("Scope the review to")
    ...
    expect(reviewPrompt).toContain("Canonical WHY Source")
    expect(reviewPrompt).toContain("Review Focus")
    expect(reviewPrompt).not.toContain("- Problem: [problem narrative]")
