...
describe("ticket-scoped work execution", () => {
  test("work can execute a ticket index or ticket artifact without reloading the whole backlog", async () => {
    ...
    expect(workPrompt).toContain("Canonical WHY source")
    expect(workPrompt).not.toContain("### Problem Narrative\n[Extracted from plan -- why this work exists]")
    ...
    expect(executionPrompt).toContain("compact WHY linkage block")
