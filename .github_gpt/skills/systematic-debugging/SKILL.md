...
## Goal
Diagnose unexpected errors through a disciplined four-phase workflow instead of trial-and-error guessing.

## Use this skill when
- A test, command, or runtime path failed unexpectedly.
- Implementation is stuck and random fixes are becoming tempting.
- A subagent retried multiple times without understanding the root cause.

## Do not use this skill when
- The error and fix are both obvious.
- You are validating a reported bug rather than debugging an active implementation failure.
- The issue is already documented in `docs/solutions/` and the documented fix directly applies.

## Non-negotiable rules
- Follow the four phases in order: Observe, Hypothesize, Test, Fix.
- Do not jump to fixes before evidence exists.
- Do not make random changes.
- Do not skip alternative hypotheses.
- Test one variable at a time.
- Fix the root cause, not the symptom.
- Verify the fix using the exact failing command or test before declaring success.
...
## Required output
Report the debugging result in this structure:
```markdown
### Debugging Session
- Symptom: ...
- Root cause: ...
