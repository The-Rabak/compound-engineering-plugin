...
## Use this skill when
- A GitHub issue or bug report needs verification and investigation.
- Reproduction steps, screenshots, code-path findings, and suggested fixes are needed.
- UI-related bugs may need browser automation evidence.

## Do not use this skill when
- The task is a simple implementation bug fix rather than investigation.
- The issue cannot be accessed and no usable description is available.
- Browser evidence is impossible and the bug is purely non-visual.

## Non-negotiable rules
- Read the issue description and comments first.
- Phase 1 research must run `repo-research-analyst` and `learnings-researcher` in parallel.
- Repeat the research loop until you have a solid hypothesis about where the bug could be coming from.
- If the bug is UI-related, verify the local server before browser reproduction.
- Capture screenshots as evidence whenever you reproduce the bug visually.
- Document findings with concrete file paths and, when possible, line references.
- Report back in a structure that can be posted to the issue.
