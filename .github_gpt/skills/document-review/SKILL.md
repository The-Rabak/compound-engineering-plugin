...
## Goal
Refine a brainstorm or plan in place so the document is clearer, leaner, and more actionable for the next workflow step.

## Use this skill when
- A brainstorm or plan document already exists and needs another refinement pass.
- The next workflow step depends on higher clarity or tighter scope.

## Do not use this skill when
- No source document exists yet.
- The task is to rewrite the whole document from scratch.
- The user wants implementation, not document refinement.

## Non-negotiable rules
- Review and update the existing document in place.
- Do not rewrite the entire document.
- Do not add new sections or requirements the user did not discuss.
- Auto-fix minor issues, but ask before making substantive changes.
- Do not create separate review files or extra metadata sections.
- Keep simplification purposeful: remove noise, not important constraints or rationale.
...
Return:
- The document path.
- The critical improvement identified.
- The changes applied.
