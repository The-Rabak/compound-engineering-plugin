...
description: >-
  This skill should be used when orchestrating multi-agent swarms using the host CLI's TeammateTool and Task system. It
...
## Goal
Coordinate a leader-led swarm of teammates and tasks so independent work can proceed in parallel without collisions.

## Use this skill when
- A multi-agent workflow needs persistent teammates, shared tasks, and message passing.
- The job benefits from divide-and-conquer execution with dependencies.
- You need explicit team lifecycle management rather than one-off subagents.

## Do not use this skill when
- A single short-lived Task subagent is enough.
- The work cannot be decomposed into safe independent units.
- You do not need persistent teammates, inboxes, or a shared task queue.

## Non-negotiable rules
- Follow the lifecycle in order: create team, create tasks, spawn teammates, coordinate, shutdown, cleanup.
- Use task dependencies to prevent race conditions.
- Do not parallelize work that touches overlapping resources without explicit serialization.
- Use inbox JSON files for inter-agent messaging.
- Use Task without team membership for short-lived work; use Task with `team_name` and `name` for persistent teammates.
- Request shutdown gracefully and clean up team artifacts at the end.
...
### the host CLI Swarm Orchestration

Master multi-agent orchestration using the host CLI's TeammateTool and Task system.
...
|-----------|-----------|---------------|
| **Agent** | A the model instance that can use tools. You are an agent. Subagents are agents you spawn. | N/A (process) |
| **Team** | A named group of agents working together. One leader, multiple teammates. | `~/.github_gpt/teams/{name}/config.json` |
...
| **Task** | A work item with subject, description, status, owner, and dependencies. | `~/.github_gpt/tasks/{team}/N.json` |
| **Inbox** | JSON file where an agent receives messages from teammates. | `~/.github_gpt/teams/{name}/inboxes/{agent}.json` |
...
Task({
  ...
  description: "Help with the host CLI",
  prompt: "How do I configure MCP servers?"
})
```
- **Tools:** Read-only + WebFetch + WebSearch
- **Best for:** Questions about the host CLI, Agent SDK, Anthropic API
...
- **Model:** Sonnet
- **Best for:** Configuring the host CLI status line
...
- `rabak-laravel-reviewer` - Laravel conventions
- `rabak-vue-reviewer` - Vue.js / Nuxt conventions
- `rabak-nest-reviewer` - NestJS conventions
- `rabak-python-reviewer` - Python best practices
- `rabak-typescript-reviewer` - TypeScript best practices
- `data-integrity-guardian` - Database and data safety
...
Task({
  ...
  prompt: "Research current best practices for JWT authentication in Laravel 2024-2026"
})
...
**Creates:**
- `~/.github_gpt/teams/feature-auth/config.json`
- `~/.github_gpt/tasks/feature-auth/` directory
...
**Removes:**
- `~/.github_gpt/teams/{team-name}/` directory
- `~/.github_gpt/tasks/{team-name}/` directory
...
`~/.github_gpt/tasks/{team-name}/1.json`:
```json
{
  "id": "1",
  "subject": "Review authentication module",
...
// cat ~/.github_gpt/teams/code-review/inboxes/team-lead.json
...
for (const file of ["AuthService.php", "User.php", "ApiController.php", "PaymentService.php"]) {
  TaskCreate({
    subject: `Review ${file}`,
...
const research = await Task({
  ...
  prompt: "Research best practices for implementing caching in Laravel APIs. Include: cache invalidation strategies, Redis vs Memcached, cache key design."
})
...
Task({
  ...
  prompt: `
    ...
    Focus on the UserController.php endpoints.
  `
})
```
...
A **backend** determines how teammate the model instances actually run. the host CLI supports three backends, and **auto-detects** the best one based on your environment.
...
the host CLI automatically selects a backend using this decision tree:
...
Teammates run as separate the model instances in tmux panes/windows.
...
**If setup fails:**
the host CLI will prompt you to set up it2 when you first spawn a teammate. You can choose to:
...
cat ~/.github_gpt/teams/{team}/config.json | jq '.members[].backendType'
...
// Read ~/.github_gpt/teams/{team}/config.json
...
```bash
# Check team config
cat ~/.github_gpt/teams/{team}/config.json | jq '.members[] | {name, agentType, backendType}'

# Check teammate inboxes
cat ~/.github_gpt/teams/{team}/inboxes/{agent}.json | jq '.'

# List all teams
ls ~/.github_gpt/teams/

# Check task states
cat ~/.github_gpt/tasks/{team}/*.json | jq '{id, subject, status, owner, blockedBy}'

# Watch for new messages
tail -f ~/.github_gpt/teams/{team}/inboxes/team-lead.json
```
...
// cat ~/.github_gpt/teams/pr-review-123/inboxes/team-lead.json
...
// Good
prompt: `
  1. Review app/Models/User.php for N+1 queries
  2. Check all Eloquent relationships have proper eager loading (with())
...
```bash
cat ~/.github_gpt/teams/{team}/inboxes/team-lead.json | jq '.'
```
...
*Based on the host CLI v2.1.19 - Tested and verified 2026-01-25*

## Required output
Return a swarm summary that includes:
- Team name and teammates.
- Task files created.
- Which tasks ran in parallel versus sequentially.
- Key findings returned through inbox messages.
- Confirmation that shutdown and cleanup completed.
