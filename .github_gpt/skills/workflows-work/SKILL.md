...
## Use this skill when
- A plan or spec is ready for execution.
- The work should be decomposed into task-sized chunks and delegated to focused subagents.
- You need persistent execution-session artifacts and disciplined quality gates before shipping.

## Do not use this skill when
- The plan is not execution-ready yet; route to `/deepen-plan` or refine it first.
- The request is only for planning or review.
- You are being asked to commit directly to the default branch without explicit permission.

## Non-negotiable rules
- Read the work document completely before doing anything else.
- Clarify ambiguities before implementation starts.
- Refuse to execute plans that do not provide task-level `Files`, `Depends on`, `Success criteria`, and `Test command` fields.
- Never commit directly to the default branch without explicit permission.
- Prefer `git-worktree` when a clean isolated branch is the right choice.
- Only parallelize tasks whose file sets do not overlap.
- Run regression tests after each completed task or task batch.
- Keep `docs/execution-sessions/` as the source of truth for progress.
- Add a `Post-Deploy Monitoring & Validation` section for every shipped change, even if the section only says no extra monitoring is required.
...
- The plan or todo path.
- The execution-session directory and `STATE.md` location.
- Completed, skipped, and remaining tasks.
- Test and lint status.
- Whether inline review, bulk review, or both were used.
- The final route to `finishing-branch` or the reason execution paused.
