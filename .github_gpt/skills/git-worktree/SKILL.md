...
## Goal
Manage isolated Git worktrees through the project manager script so branch setup stays safe and consistent.

## Use this skill when
- You need a new isolated branch workspace for review or implementation.
- Multiple parallel branches or reviews should not share one checkout.
- Worktree listing, switching, env copying, or cleanup is required.

## Do not use this skill when
- A normal branch in the current checkout is sufficient.
- You are about to perform raw `git worktree` operations manually.
- No isolated workspace is needed.

## Non-negotiable rules
- Never run `git worktree add` directly.
- Always use `bash ${GITHUB_GPT_ROOT}/skills/git-worktree/scripts/worktree-manager.sh ...`.
- The manager script is responsible for copying `.env`, `.env.local`, `.env.test`, and related files.
- The manager script must ensure `.worktrees` is ignored properly.
- Use the script for create, list, switch, copy-env, and cleanup.
...
bash ${GITHUB_GPT_ROOT}/skills/git-worktree/scripts/worktree-manager.sh create feature-name
...
bash ${GITHUB_GPT_ROOT}/skills/git-worktree/scripts/worktree-manager.sh create feature-login

# List all worktrees
bash ${GITHUB_GPT_ROOT}/skills/git-worktree/scripts/worktree-manager.sh list

# Switch to a worktree
bash ${GITHUB_GPT_ROOT}/skills/git-worktree/scripts/worktree-manager.sh switch feature-login

# Copy .env files to an existing worktree (if they weren't copied)
bash ${GITHUB_GPT_ROOT}/skills/git-worktree/scripts/worktree-manager.sh copy-env feature-login

# Clean up completed worktrees
bash ${GITHUB_GPT_ROOT}/skills/git-worktree/scripts/worktree-manager.sh cleanup
```
...
```bash
# the host CLI recognizes you're not on the PR branch
# Offers: "Use worktree for isolated review? (y/n)"

# You respond: yes
# Script runs (copies .env files automatically):
bash ${GITHUB_GPT_ROOT}/skills/git-worktree/scripts/worktree-manager.sh create pr-123-feature-name
...
bash ${GITHUB_GPT_ROOT}/skills/git-worktree/scripts/worktree-manager.sh create feature-notifications
...
```bash
bash ${GITHUB_GPT_ROOT}/skills/git-worktree/scripts/worktree-manager.sh copy-env feature-name
```
...
## Required output
Return the operation result, including:
- The worktree name.
- The branch name.
- The filesystem path.
