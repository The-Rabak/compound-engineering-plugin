...
## Goal
Execute the `Workflow: Create a New Skill` workflow directly and completely.
...
```bash
mkdir -p ~/.github_gpt/skills/{skill-name}
# If complex:
mkdir -p ~/.github_gpt/skills/{skill-name}/workflows
mkdir -p ~/.github_gpt/skills/{skill-name}/references
# If needed:
mkdir -p ~/.github_gpt/skills/{skill-name}/templates  # for output structures
mkdir -p ~/.github_gpt/skills/{skill-name}/scripts    # for reusable code
```
...
```bash
cat > ~/.github_gpt/commands/{skill-name}.md << 'EOF'
---
