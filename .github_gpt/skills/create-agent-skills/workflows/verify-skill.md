...
## Goal
Execute the `Workflow: Verify Skill Content Accuracy` workflow directly and completely.
...
cat ~/.github_gpt/skills/{skill-name}/workflows/*.md 2>/dev/null
cat ~/.github_gpt/skills/{skill-name}/references/*.md 2>/dev/null
```
