...
## Description
Skills use pure XML structure for consistent parsing, efficient token usage, and improved the model performance.
...
## Parseability
XML provides unambiguous boundaries and semantic meaning. the model can reliably:
...
Markdown headings are just visual formatting. the model must infer meaning from heading text, which is less reliable.
...
## Success criteria
```
Total: ~20 tokens, no semantic meaning to the model
...
the model performs better with pure XML because:
...
Assume the model is smart. Don't explain obvious concepts.
...
The concise version assumes the model knows what PDFs are, understands Python imports, and can read code. All those assumptions are correct.
...
## Description
Match the level of specificity to the task's fragility and variability. Give the model more freedom for creative tasks, less freedom for fragile operations.
...
the model has freedom to adapt the review based on what the code needs.
...
the model can customize the template based on requirements.
...
the model must follow the exact command with no variation.
...
## Description
SKILL.md serves as an overview. Reference files contain details. the model loads reference files only when needed.
...
## Description
Validation scripts are force multipliers. They catch errors that the model might miss and provide actionable feedback.
...
## Principle Summary
## Xml Structure
Use pure XML structure for consistency, parseability, and the model performance. Required tags: objective, quick_start, success_criteria.

## Conciseness
Only add context the model doesn't have. Assume the model is smart. Challenge every piece of content.
