...
Custom slash commands have been merged into skills. A file at `.github_gpt/commands/review.md` and a skill at `.github_gpt/skills/review/SKILL.md` both create `/review` and work the same way. Existing `.github_gpt/commands/` files keep working. Skills add optional features: a directory for supporting files, frontmatter to control invocation, and automatic context loading.
...
| `description` | Recommended | What it does AND when to use it (max 1024 chars). the model uses this to decide when to apply the skill. |
| `argument-hint` | No | Hint shown during autocomplete. Example: `[issue-number]` or `[filename] [format]` |
| `disable-model-invocation` | No | Set `true` to prevent the model from auto-loading. Use for manual workflows. Default: `false` |
| `user-invocable` | No | Set `false` to hide from `/` menu. Use for background knowledge. Default: `true` |
| `allowed-tools` | No | Tools the model can use without permission prompts. Example: `Read, Bash(git *)` |
...
| Frontmatter | User can invoke | the model can invoke | When loaded into context |
|-------------|----------------|-------------------|--------------------------|
...
| Personal | `~/.github_gpt/skills/<name>/SKILL.md` | You, across all projects |
| Project | `.github_gpt/skills/<name>/SKILL.md` | Anyone working in repository |
...
1. **Discovery**: the model loads only name and description at startup (2% of context window budget)
2. **Activation**: When your request matches a skill's description, the model loads the full content
3. **Execution**: the model follows the skill's instructions
...
Commands execute immediately and their output replaces the placeholder. the model only sees the final result.
...
- **Project skills**: Commit `.github_gpt/skills/` to version control
