...
## Principle
When the model performs complex, open-ended tasks, create a plan in a structured format, validate it, then execute.
...
Specific errors help the model fix issues without guessing.
...
## Principle
Guide the model through decision points with clear branching logic.
...
- Guiding the model to correct path improves outcomes

## Validation Scripts
## Principles
Validation scripts are force multipliers. They catch errors that the model might miss and provide actionable feedback for fixing issues.
...
Verbose errors help the model fix issues in one iteration instead of multiple rounds of guessing.
...
Actionable suggestions show the model exactly what to fix.
...
## Checkpoint Pattern
## Principle
For long workflows, add checkpoints where the model can pause and verify progress before continuing.
...
## Error Recovery
## Principle
Design workflows with clear error recovery paths. the model should know what to do when things go wrong.
