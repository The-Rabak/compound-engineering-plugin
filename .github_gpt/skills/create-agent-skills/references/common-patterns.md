...
## Flexible Guidance
Use when the model should adapt the format based on context:
...
## Principle
Choose one term and use it throughout the skill. Inconsistent terminology confuses the model and reduces execution quality.
...
the model must now interpret: Are "API routes" and "URLs" the same? Are "fields", "boxes", "elements", and "controls" the same?
...
the model must now research and compare all options before starting. This wastes tokens and time.
...
**Why it matters**: Vague descriptions prevent the model from discovering and using the skill appropriately.
...
**Why it matters**: the model may only partially read deeply nested files. Keep references one level deep from SKILL.md.
...
## Description
Keep SKILL.md concise by linking to detailed reference files. the model loads reference files only when needed.
...
- the model only reads relevant reference files
...
- the model can fix issues without guessing
- Specific error messages reduce iteration cycles
- Available options shown in error messages

## Checklist Pattern
## Description
For complex multi-step workflows, provide a checklist the model can copy and track progress.
