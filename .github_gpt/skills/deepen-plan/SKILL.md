...
## Goal
Enhance an existing plan with deeper research, better execution structure, and matched skill and learning context.

## Use this skill when
- A plan exists and needs more detail, stronger grounding, or better execution readiness.
- `/workflows-work` compatibility needs to be improved.
- The plan would benefit from applying discovered skills and prior learnings.

## Do not use this skill when
- No plan file exists yet; create one with `/workflows-plan` first.
- The request is implementation, not plan enhancement.
- You only need a light editorial pass; use `document-review` instead.

## Non-negotiable rules
- Do not proceed until you have a valid plan path.
- Preserve the plan's existing structure while enhancing it.
- Validate execution readiness for every task.
- Split tasks that are too large for reliable subagent execution.
- If `source_docs` exists, you must re-fetch the full source documents, not just reuse summaries.
- Dynamically discover skills from every available source. Do not assume only one plugin matters.
- Spawn one subagent per matched skill, and run all matched skill subagents in parallel.
- Spawn one subagent per relevant learning file, and run those learning checks in parallel.
...
## Required output
Return an enhancement summary that includes:
- The plan path updated.
- Execution-readiness score before and after.
- Any task splits introduced.
- The matched skills used.
- The learning documents applied.
- The major sections that were strengthened.
