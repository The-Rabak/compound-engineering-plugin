...
## Goal
Manage the file-based todo tracking system in `todos/` using the standard filename, frontmatter, section, and lifecycle rules.

## Use this skill when
- You need to create, triage, update, or complete markdown todo files in `todos/`.
- Findings from review, feedback, or execution need durable tracking.
- Dependencies and work-log history must be recorded on disk.

## Do not use this skill when
- The work belongs in a database-backed Todo model or another application-level system.
- You only need temporary in-memory tracking; use TodoWrite instead.
- The issue is trivial enough to fix immediately without creating tracked work.

## Non-negotiable rules
- Use the filename pattern `{issue_id}-{status}-{priority}-{description}.md`.
- Issue ids are sequential, zero-padded, and never reused.
- Valid statuses are `pending`, `ready`, and `complete`.
- Valid priorities are `p1`, `p2`, and `p3`.
- Use the todo template and keep the required sections intact.
- Record dependencies in frontmatter.
- Add work-log entries whenever triaging, working, or completing a todo.
- Distinguish this markdown todo system from any Laravel Todo model and from TodoWrite.
...
**By:** the host CLI / Developer Name
...
**Laravel Todo model:**
- Database model in `app/Models/Todo.php`
...
## Required output
When you create or update todos, report:
- The filename or filenames affected.
- The new status.
- Any dependencies added or cleared.
- Any follow-up todos that are now unblocked.
