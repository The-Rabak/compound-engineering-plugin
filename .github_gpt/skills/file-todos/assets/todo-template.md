...
## Purpose
Use this asset as a reusable starting point for `file-todos`.
...
**Example format:**
- Identified 12 missing test scenarios in `tests/Unit/UserTest.php`
- Current coverage: 60% of code paths
- Missing: empty inputs, special characters, large payloads
- Similar issues exist in `tests/Unit/PostTest.php` (~8 scenarios)
...
**Affected files:**
- `app/Models/User.php:45` - full_name method
- `app/Services/UserService.php:12` - validation logic
- `tests/Unit/UserTest.php` - existing tests
...
- **PR:** #1287
- **Related issue:** #456
- **Error log:** [link to error log or Horizon dashboard]
