...
## Goal
Close out a feature branch with final verification, the correct shipping path, and cleanup of related artifacts.

## Use this skill when
- Implementation is complete and the branch is ready to ship, merge, keep for review, or discard.
- `/workflows-work` is handing off to final branch completion.

## Do not use this skill when
- Work is still in progress.
- You are already on the default branch with nothing to finish.
- Tests or linting are failing and have not been fixed yet.

## Non-negotiable rules
- Run final tests and linting before any shipping action.
- Do not proceed if tests or linting fail.
- Stage files selectively; do not blindly commit everything.
- Ask the user which completion path they want unless the workflow is fully orchestrated and must default to PR creation.
- Include `Post-Deploy Monitoring & Validation` in the PR or MR description for every change.
- If a worktree was used, clean it up.
- Update the plan and execution session status if those artifacts exist.
...
## Required output
Return a branch-complete summary with:
- The branch name.
- The action taken.
- Test and lint results.
- Commit count or final commit.
- Files changed.
- PR or MR link if one was created.
- Worktree cleanup status.
- Plan and execution-session status updates.
