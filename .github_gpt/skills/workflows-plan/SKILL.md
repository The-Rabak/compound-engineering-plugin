...
## Use this skill when
- A feature, bug fix, or improvement needs a structured plan.
- You need to transform a brainstorm, spec, ticket, or plain-language request into a plan others can execute.
- The output must be compatible with `/workflows-work`.

## Do not use this skill when
- The request is still too unclear to plan; route to `/workflows-brainstorm` first.
- The task is implementation rather than planning.
- The user only wants a light edit to an existing brainstorm or plan; use `document-review`.

## Non-negotiable rules
- Do not proceed without a clear feature description, plan path, or equivalent source input.
- If the input is an existing markdown plan or spec, preserve its structure and enrich it instead of replacing it.
- If a relevant brainstorm from the last 14 days exists, use it as planning context.
- Store structured inputs in plan frontmatter under `source_docs` and preserve that data.
- Stay focused on WHAT must be delivered. Do not over-specify implementation details that belong in execution.
- Every implementation task intended for `/workflows-work` must include `Files`, `Depends on`, `Success criteria`, and `Test command`.
- When multiple independent documents or agents can run at once, you must run them in parallel.
- Always run `spec-flow-analyzer` before finalizing the plan.
...
- [ ] Add collapsible sections for lengthy logs or optional details using markdown details/summary blocks
...
- [ ] Note which AI tools were used for initial exploration (the model, Copilot, etc.)
...
6. **Start `/workflows-work` on remote** - Begin implementing in the host CLI on the web (use `&` to run in background)
...
- **`/workflows-work` on remote** -> Run `/workflows-work docs/plans/.md &` to start work in background for the host CLI web
...
- YAML frontmatter with `title`, `type`, `status`, `date`, and `source_docs`.
- Problem statement and project context.
- Acceptance criteria.
- Execution-ready implementation tasks with `Files`, `Depends on`, `Success criteria`, and `Test command`.
- References and any important research or institutional learnings.
