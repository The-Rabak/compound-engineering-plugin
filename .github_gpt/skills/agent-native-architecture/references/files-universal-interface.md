...
## Goal
Use this reference to support `agent-native-architecture` with direct, decision-oriented guidance.
...
Agents are naturally good at files. the host CLI works because bash + filesystem is the most battle-tested agent interface. When building agent-native apps, lean into this.
