...
## Goal
Review pending findings one by one and decide which ones become ready todos in the file-based todo system.

## Use this skill when
- You need to approve, skip, or modify pending findings in `todos/`.
- Code review findings, security findings, performance findings, or other issues need file-based triage.
- The next step depends on whether each item should become actionable tracked work.

## Do not use this skill when
- The task is to implement fixes; use a resolver workflow instead.
- The task is to create brand new findings from review output; use the upstream review flow first.
- You want to batch-edit plan files or code during triage.

## Non-negotiable rules
- Set the model to Haiku before triage starts.
- Do not code anything during triage.
- Read all pending todo files before presenting the queue.
- Present findings one at a time.
- When approving an existing pending todo, rename the file from `pending` to `ready` and update frontmatter to `status: ready`.
- When approving a new todo, use the correct naming convention and populate the required sections.
- Update the `Work Log` with the triage approval entry.
- When skipping an item, delete the todo file and track it in the summary.
- Keep visible progress through the queue until every item is processed.
...
## Required output
Return a final triage summary with:
- Total items processed.
- Count of approved ready todos.
- Count of skipped items.
- Ready todo filenames.
- Deleted filenames or skipped-item notes.
- Suggested next commands, including viewing ready todos or running `/resolve_todo_parallel`.
