...
import { expandHome, resolveTargetHome, stripSurroundingQuotes } from "../utils/resolve-home"
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    try {
      ...
      const primaryOutputRoot = resolveTargetOutputRoot(targetName, outputRoot, codexHome, cursorHome, piHome, hasExplicitOutput)
      ...
      for (const extra of extraTargets) {
        ...
        const extraRoot = resolveTargetOutputRoot(extra, path.join(outputRoot, extra), codexHome, cursorHome, piHome, hasExplicitOutput)
...
async function resolvePluginPath(input: string): Promise<ResolvedPluginPath> {
  ...
  if (input.startsWith(".") || input.startsWith("/") || input.startsWith("~") || /^[a-zA-Z]:[\\/]/.test(input)) {
...
function parseExtraTargets(value: unknown, primaryTarget?: string): string[] {
  ...
  const allowedTargets = new Set(getTargetNamesForSurface("install").filter((targetName) => targetName !== "opencode"))
...
function resolveOutputRoot(value: unknown): string {
  if (value && String(value).trim()) {
    const expanded = expandHome(stripSurroundingQuotes(String(value)))
...
function resolveGitHubSource(): string {
  ...
  return "https://github.com/The-Rabak/compound-engineering-plugin"
}
