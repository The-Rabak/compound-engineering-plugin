...
4. **Read only the fixed-core fields for the artifact's own `kind`.** The parsed object's Tier 1 envelope + that kind's Tier 2 contract core (the matching entry in `REQUIRED_KEYS_BY_KIND` in `island-contract.md`) are the load-bearing facts. For `kind: "plan"`: `title`, `type`, `date`, `status`, `refs.*`, `execution_shape`, `tdd`, `runtime_stack`, `constitution`, `handoff`, `slices[]`, `success_criteria[]`, `suggested_e2e_suite[]`, plus the Tier 3 prose fields (`problem_narrative`, `user_story`, `architectural_context`, `specified_scope_contract`, `references`). For `kind: "brainstorm"` (T03): `title`, `type`, `date`, `status`, `refs.*`, `problem_narrative`, `user_story`, `architectural_context`, `success_criteria[]`, `chosen_approach`, `key_decisions[]`, `resolved_questions[]`, `open_questions[]`, `handoff`, plus the Tier 3 prose fields (`scope_boundary`, `non_goals`, `constitution_alignment`, `approaches_considered`, `stakeholder_impact`). For `kind: "architecture"` (T04): `title`, `type`, `date`, `status`, `refs.*`, `plan_ref`, `feature_homes[]`, `shared_global_decisions[]`, `deepening_candidates[]`, `context_tiers`, `deletion_test[]`, `interfaces_as_test_surfaces[]`, `seams_adapters_contracts[]`, `drift_checks[]`, `recommendations` (`{deepen_plan[], work[], review[]}`), `handoff` (this kind's own three-key `{deepen_plan, work, review}` shape — not the four-key shape `plan`/`brainstorm` use), plus the Tier 3 prose fields (`purpose_linkage`, `module_blueprint`, `design_it_twice`, `review_depth`, `open_questions`). Use `island-contract.md`'s field-coverage map for the artifact's `kind` to translate a legacy frontmatter key or section name into its island home.
