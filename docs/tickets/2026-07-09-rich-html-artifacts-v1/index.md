...
- **Go/no-go gate:** T05 is the pilot's go/no-go. If T05's L3/L4 gate fails, **do not proceed to T06** (release) — send the failure back for repair. A failed gate blocks release by design.
  ...
  - **Re-affirmed (2026-07-13) from durable, re-runnable evidence — todo 021 Option 1.** The L3 plan `.html`↔`.md` equivalence and the L4 fail-loud drill were re-run as live agent sessions and committed under `docs/verification/2026-07-13-html-artifacts-equivalence/` (`STATE.md` + `run-l3-diff.ts`; non-gitignored). Independent re-run: both arms `floor_pass`, cross-arm `diffs: []` over all packet fields + ticket-id set + edges + batches; all four L4 fail-loud codes reproduced with literal diagnostics; `bun test` 291 pass. This replaces the prose-only affirmation above with re-checkable artifacts. (Separately, the v2 brainstorm→plan arm surfaced an `architectural_context` coverage gap — tracked as `todos/029`; it does not affect this v1 plan-arm gate.)
